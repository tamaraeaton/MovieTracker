...
export default class MovieList extends Component {
    ...
    // Per Austin, we should not be using this lifecycle method
    componentDidUpdate() {
        axios.get('http://localhost:8080/movieTracker/movies/' + this.props.username)
            .then(res => {
                this.setState({
                ...
                // console.log("res.data", res.data)
                // THIS REPEATS THE DATA INDEFINATELY, BUT STILL WORKS
            })
            .catch(err => {
                console.log(err)
            })
    ...
    render() {
        return (
            <div>
                <h2>Movie List</h2>
                <div id="movie-list">
                <table id="table-list" className="table table-striped" style={{ marginTop: 20 }}>
                    <thead>
